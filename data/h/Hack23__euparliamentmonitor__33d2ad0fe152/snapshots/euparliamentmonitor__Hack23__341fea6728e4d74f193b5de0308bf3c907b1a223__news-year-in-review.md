...
on:
  schedule:
    - cron: "0 8 15 1 *"  # 1st of each month around 08:00 UTC
  workflow_dispatch:
    inputs:
      force_generation:
...
#      point at which Stage C must hand off to Stage D (or skip Stage D and
#      jump to Stage E). The minute-22 elapsed-time tripwire in the prompt
#      body enforces this regardless of GREEN/RED — even a freshly-emitted
#      GREEN gate at minute 22 must fall through to ANALYSIS_ONLY so Stage
#      D + E still have reserved time before the PR-call deadline. Local
#      stage ceilings (A ≤ 4, B 12–15, C ≤ 3) sum to 22 — the tripwire
#      backstops any per-stage overrun.
#
#   2. **safe-outputs PR-call deadline = minute ≤ 25 (target ≤ 22).** This
#      is the deadline for the single safe-outputs `create_pull_request`
#      call. Stage D ≤ 2 min and Stage E ≤ 1–2 min must fit between the
#      Stage C exit (≤ minute 22) and this deadline.
#
# Tightened from 22–27 min after run #24957585804 emitted no safe outputs
# (agent succeeded but PR call never landed) — same root cause as
# news-month-in-review run #24954208628 fixed in #1444. The 365-day
# forward data window naturally produces more data than week-ahead, so
# Stage A/B ceilings are tighter here than in the other unified
# workflows. ~20 min of headroom remains for npm setup, render, and git
# push under the 45-min cap.
timeout-minutes: 45
...
tools:
  ...
  cache-memory:
    key: news-year-in-review-${{ github.repository_owner }}
    retention-days: 7
    ...
    steps:
      ...
      - name: Run host-side PAT PR fallback
        env:
          ...
          GH_AW_PAT_FALLBACK_SLUG: year-in-review
          GH_AW_PAT_FALLBACK_WORKFLOW_NAME: "News: EU Parliament Year In Review — Unified"
...
engine:
  ...
  max-continuations: 1
---
# 📰 EU Parliament Year In Review — Unified Workflow
...
| Parameter | Value |
|-----------|-------|
| `ARTICLE_TYPE_SLUG` | `year-in-review` |
| Family | **Unified** (Stages A → B → C → D → E in one workflow) |
| Data window | next trailing 365 days |
...
| Stage A budget | **≤ 4 min** (tighter than other workflows — 365-day forward data window is larger) |
...
| **Hard PR-call deadline** | **minute ≤ 25 elapsed** (target ≤ 22) — deadline for the single safe-outputs `create_pull_request` call. After this, the safeoutputs MCP HTTP session is at risk of being reaped. |
...
> tools does **not** refresh it). Run #24957585804 emitted **zero** safe
> outputs — the agent succeeded but the PR call never landed inside the
> session window — same root cause as `news-month-in-review` run
> #24954208628 fixed in #1444. The schedule has **two distinct deadlines**:
>
> - **Stage C exit by minute ≤ 22** (Stage A ≤ 4 + Stage B 12–15 + Stage
>   C ≤ 3 = 22 min ceiling) — backstopped by the elapsed-time tripwire
>   below.
> - **Single PR call by minute ≤ 25** (Stage D ≤ 2 + Stage E ≤ 1–2 =
>   ~3–4 min after the Stage C exit) — well below the ~28–30 min
...
- Economic context (**IMF only** for macro/fiscal/monetary/trade) is mandatory — annual retrospectives always touch macro/policy.
...
- Mandates the full Family-D set including [`.github/prompts/12-electoral-cycle.md`](../prompts/12-electoral-cycle.md): `term-arc.md`, `mandate-fulfilment-scorecard.md`, `legislative-pipeline-forecast.md`.
- Schedule `0 8 15 1 *` (mid-January) waits for EP roll-call publication delay across the entire prior calendar year.
- Mine prior-run forward statements (per `01-data-collection.md` §8); resolve every expired carry-forward statement (`status` ∈ {`resolved`, `stale`, `extended`}) per [`forward-projection-methodology.md` §6](../../analysis/methodologies/forward-projection-methodology.md#6-carry-forward--forward-statement-quality-gate).
...
LAST_YEAR=$(date -u -d 'trailing 365 days ago' +%Y-%m-%d)
RUN_EPOCH=$(date -u +%s)
RUN_ID="year-in-review-run$$-$RUN_EPOCH"
...
ANALYSIS_DIR=$(scripts/resolve-analysis-dir.sh "$TODAY" year-in-review)
WORKFLOW_START_EPOCH=$RUN_EPOCH
echo "ARTICLE_TYPE_SLUG=year-in-review"                  >> "$GITHUB_ENV"
...
> always derive dates from `$TODAY` / `$LAST_WEEK` / `$LAST_YEAR`. Never
> hard-code a year.
...
```bash
HORIZON_END=$(date -u -d 'trailing 365 days' +%Y-%m-%d)
...
**Multi-day foreseen activities fan-out (mandatory — all plenary session days in the 365-day window):**

For each upcoming plenary session in the next trailing 365 days, identify the session
...
top-level `articleType: year-in-review` and every produced file under `files.*`.
...
> **⏱️ Elapsed-Time Tripwire (year-in-review specific)**: At the top of
...
> straight to Stage E. The 365-day forward data window naturally pushes
...
#    Emits news/<TODAY>-year-in-review.en.md (canonical aggregated markdown) plus
#    news/<TODAY>-year-in-review-en.html (rendered article). Always
...
- `base: "main"`
- `head: "news/${TODAY}-year-in-review-${RUN_ID}"`
- `title: "[news] year-in-review — ${TODAY} (run ${RUN_ID})"` *(append `(analysis-only)` suffix when `GATE_RESULT=ANALYSIS_ONLY`)*
...
- Never author article prose — the deterministic renderer owns Stage D.
  Agent edits to `news/<date>-year-in-review.en.md` after `npm run generate-article`
  exits are forbidden.
