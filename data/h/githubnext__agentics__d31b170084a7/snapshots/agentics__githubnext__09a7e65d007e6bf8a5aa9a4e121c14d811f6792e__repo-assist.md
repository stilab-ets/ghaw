...
safe-outputs:
  messages:
    footer: "> Generated by 🌈 Repo Assist at [{run-started}]({run_url}). [Install Repo Assist in your repo](https://github.com/githubnext/agentics/blob/main/docs/repo-assist.md)."
    run-started: "Repo Assist is processing {event_type}..."
    run-success: "✓ Repo Assist completed successfully"
    run-failure: "✗ Repo Assist encountered {status}"
  add-comment:
    max: 10
    target: "*"
