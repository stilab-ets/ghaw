...
grep -rl "Filed by: Unknown" news/ | grep "motions" | wc -l || true

# Check for untranslated spans in English article (should return 0)
grep -c 'data-translate="true"' "news/$(date +%Y-%m-%d)-motions-en.html" 2>/dev/null || true

# Check word count of English article (must be >= 500; note: includes HTML tags)
wc -w "news/$(date +%Y-%m-%d)-motions-en.html" 2>/dev/null || true

# Check for duplicate "Why It Matters" content (should return empty)
grep -o 'Why It Matters[^<]*' "news/$(date +%Y-%m-%d)-motions-en.html" 2>/dev/null | sort | uniq -d || true
```
