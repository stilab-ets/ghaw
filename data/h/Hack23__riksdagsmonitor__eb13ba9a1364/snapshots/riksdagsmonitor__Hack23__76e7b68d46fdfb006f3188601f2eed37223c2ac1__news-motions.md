...
npx tsx scripts/pre-article-analysis.ts --date "$ARTICLE_DATE" --limit 50 --doc-type motions > /tmp/pipeline-output.log 2>&1
PIPE_EXIT=$?
...
npx tsx scripts/catalog-downloaded-data.ts --pending-only --type motions 2>/dev/null | jq -r '.pendingAnalysis // 0' > /tmp/pending_num.txt 2>/dev/null || echo 0 > /tmp/pending_num.txt
PENDING=0
...
# CRITICAL: Do NOT run htmlhint/--fix on all news/*-*.html — that modifies 360+ existing
# motions articles which then get staged and exceed the 100-file PR limit (E003).
...
grep -c \'data-translate="true"\' "news/$ARTICLE_DATE-opposition-motions-en.html" 2>/dev/null || true

# Check word count of English article text content (warn if < 500; HTML tags stripped)
FILE="news/$ARTICLE_DATE-opposition-motions-en.html"
...
grep -o 'Why It Matters[^<]*' "news/$ARTICLE_DATE-opposition-motions-en.html" 2>/dev/null | sort | uniq -d || true
```
