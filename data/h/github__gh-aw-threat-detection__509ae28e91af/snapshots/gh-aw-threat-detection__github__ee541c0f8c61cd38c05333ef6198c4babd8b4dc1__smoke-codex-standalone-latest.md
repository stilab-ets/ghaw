...
permissions:
  ...
  pull-requests: read
  actions: read
name: Smoke Codex Standalone Latest
engine: codex
strict: false
features:
...
safe-outputs:
  ...
  create-issue:
    ...
    close-older-key: "smoke-codex-standalone-latest"
...
> `smoke-codex-standalone.md`. Its `.lock.yml` is regenerated per the
...
Always create an issue titled `Smoke Test: Codex Standalone Latest - ${{ github.run_id }}` with:
- ✅ or ❌ for each test
