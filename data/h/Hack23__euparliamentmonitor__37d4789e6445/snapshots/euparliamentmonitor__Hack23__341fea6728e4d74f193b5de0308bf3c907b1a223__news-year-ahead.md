---
name: "News: EU Parliament Year Ahead — Unified"
description: Generates a single PR containing analysis artifacts and the rendered year-ahead article (Stages A → B → C → D → E in one workflow).
strict: false
on:
  schedule:
...
| Parameter | Value |
|-----------|-------|
| `ARTICLE_TYPE_SLUG` | `year-ahead` |
| Family | **Unified** (Stages A → B → C → D → E in one workflow) |
| Data window | next 365 days |
...
- Economic context (**IMF only** for macro/fiscal/monetary/trade) is mandatory — annual previews always touch macro/policy.
...
LAST_YEAR=$(date -u -d '365 days ago' +%Y-%m-%d)
RUN_EPOCH=$(date -u +%s)
RUN_ID="year-ahead-run$$-$RUN_EPOCH"
...
ANALYSIS_DIR=$(scripts/resolve-analysis-dir.sh "$TODAY" year-ahead)
WORKFLOW_START_EPOCH=$RUN_EPOCH
echo "ARTICLE_TYPE_SLUG=year-ahead"                  >> "$GITHUB_ENV"
...
```bash
HORIZON_END=$(date -u -d '365 days' +%Y-%m-%d)
...
For each upcoming plenary session in the next 365 days, identify the session
...
top-level `articleType: year-ahead` and every produced file under `files.*`.
...
> **⏱️ Elapsed-Time Tripwire (year-ahead specific)**: At the top of
...
#    Emits news/<TODAY>-year-ahead.en.md (canonical aggregated markdown) plus
#    news/<TODAY>-year-ahead-en.html (rendered article). Always
...
- `base: "main"`
- `head: "news/${TODAY}-year-ahead-${RUN_ID}"`
- `title: "[news] year-ahead — ${TODAY} (run ${RUN_ID})"` *(append `(analysis-only)` suffix when `GATE_RESULT=ANALYSIS_ONLY`)*
...
- Never author article prose — the deterministic renderer owns Stage D.
  Agent edits to `news/<date>-year-ahead.en.md` after `npm run generate-article`
  exits are forbidden.
