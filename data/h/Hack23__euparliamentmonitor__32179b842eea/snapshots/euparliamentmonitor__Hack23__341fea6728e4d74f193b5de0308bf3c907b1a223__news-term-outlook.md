...
# news-month-in-review run #24954208628 fixed in #1444. The term-end
...
tools:
  ...
  cache-memory:
    key: news-term-outlook-${{ github.repository_owner }}
    retention-days: 7
    ...
    steps:
      ...
      - name: Run host-side PAT PR fallback
        env:
          ...
          GH_AW_PAT_FALLBACK_SLUG: term-outlook
          GH_AW_PAT_FALLBACK_WORKFLOW_NAME: "News: EU Parliament Term Outlook — Unified"
...
TERM_BACK=$(date -u -d 'run date to next EP election week ago' +%Y-%m-%d)
RUN_EPOCH=$(date -u +%s)
RUN_ID="term-outlook-run$$-$RUN_EPOCH"
...
ANALYSIS_DIR=$(scripts/resolve-analysis-dir.sh "$TODAY" term-outlook)
WORKFLOW_START_EPOCH=$RUN_EPOCH
echo "ARTICLE_TYPE_SLUG=term-outlook"                  >> "$GITHUB_ENV"
...
> always derive dates from `$TODAY` / `$LAST_WEEK` / `$TERM_BACK`. Never
> hard-code a year.
...
top-level `articleType: term-outlook` and every produced file under `files.*`.
...
> **⏱️ Elapsed-Time Tripwire (term-outlook specific)**: At the top of
...
> straight to Stage E. The term-end forward data window naturally pushes
...
#    Emits news/<TODAY>-term-outlook.en.md (canonical aggregated markdown) plus
#    news/<TODAY>-term-outlook-en.html (rendered article). Always
...
- `base: "main"`
- `head: "news/${TODAY}-term-outlook-${RUN_ID}"`
- `title: "[news] term-outlook — ${TODAY} (run ${RUN_ID})"` *(append `(analysis-only)` suffix when `GATE_RESULT=ANALYSIS_ONLY`)*
...
- Never author article prose — the deterministic renderer owns Stage D.
  Agent edits to `news/<date>-term-outlook.en.md` after `npm run generate-article`
  exits are forbidden.
