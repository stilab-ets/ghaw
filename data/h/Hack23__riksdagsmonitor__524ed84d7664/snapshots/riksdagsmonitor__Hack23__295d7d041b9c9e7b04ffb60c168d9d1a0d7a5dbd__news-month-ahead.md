...
npx playwright test tests/validate-articles-playwright.ts --grep "month-ahead" 2>/dev/null || echo "⚠️ Playwright validation skipped (not installed)"

# Validate JSON-LD cross-references
npx tsx scripts/validate-cross-references.ts news/*-month-ahead-*.html 2>/dev/null || echo "⚠️ Cross-reference validation skipped"
```
