...
description: Generates month-ahead strategic outlook articles and renders HTML in all 14 supported languages in a single agentic run via executive-brief cascade. Runs on 1st of each month.
strict: false
imports:
...
- **Rendered HTML**: `news/$ARTICLE_DATE-month-ahead-{en,sv,da,no,fi,de,fr,es,nl,ar,he,ja,ko,zh}.html` — **always all 14 languages** (produced by `scripts/render-articles.ts`)
