...
**Statistical enrichment:** SCB/World Bank — for major economic milestones (budget debates, economic policy events), pre-fetch trend data from committee-mapped indicators. See `scripts/scb-context.ts` for 15 domain→committee mappings and `scripts/world-bank-context.ts` for 28 WB indicator→committee mappings. Full indicator inventory: `analysis/worldbank/indicators-inventory.json`. See `SHARED_PROMPT_PATTERNS.md` §"WORLD BANK ECONOMIC CONTEXT INTEGRATION" for Chart.js chart templates (`economic-comparison`, `economic-trend`, `nordic-radar`). MUST generate ≥2 economic charts for monthly forecasting.
