...
Uses `memory/news-generation` branch. START: read `memory/news-generation/last-run-news-month-ahead.json` + `covered-documents/{YYYY-MM-DD}.json`. END: update both + `translation-status.json`. Skip already-covered dok_ids.
