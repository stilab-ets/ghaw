...
- ✅ Legislative risk analysis (name specific procedures, rapporteurs, political group positions)
- ✅ Risk outlook (≥200 words with probability-labelled scenarios and institutional risks)
- ✅ World Bank economic context when legislation has economic/trade/employment dimension
- ✅ Chart/dashboard data for ≥1 data visualization with real data

> **⚠️ ANALYSIS-TO-ARTICLE DATA CONTRACT (AI-First)**: Per [SHARED_PROMPT_PATTERNS.md §Analysis-to-Article Data Contract](../prompts/SHARED_PROMPT_PATTERNS.md#-analysis-to-article-data-contract-ai-first), analysis markdown is **NEVER parsed by scripts**. YOU read every `analysis/daily/<date>/propositions-run<id>/**/*.md` file (especially `existing/stakeholder-impact.md`, `classification/impact-matrix.md`, `intelligence/stakeholder-map.md`, `intelligence/synthesis-summary.md`) as context and author each stakeholder card, outcome-matrix `reason` cell, impact-assessment dimension, amendment-lineage narrative, `why` text, and `outlook` prose directly in the rendered English HTML. **FORBIDDEN** to ship the `[AI_ANALYSIS_REQUIRED]` sentinel, any of the six generic stakeholder-reasoning fallback sentences, or date-range topic placeholders. The HTML fallback-leak validator enforces this at publish time — see the Validator Enforcement command below.
...
ARTICLE_HTML=$(ls -t "news/${TODAY}-propositions"*"-en.html" 2>/dev/null | head -1)
...
// ↳ FALLBACK if 404/timeout: european_parliament___get_plenary_documents({ year: <current-year>, limit: 50 }) + european_parliament___get_committee_documents({ limit: 50 })
...
> **🔴 VOTING EVIDENCE REQUIREMENT**: Any analysis that claims political group voting positions (e.g., "broad cross-party support") MUST cite actual data from `get_voting_records` or `get_meeting_decisions`. If voting records are unavailable, mark coalition claims as LOW confidence.
...
- ✅ **World Bank economic data included** when legislation has policy/economic dimension
