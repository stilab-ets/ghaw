...
- **Minutes 8–15**: Query MCP tools for interpellation data
...
> 2. Stage and commit locally: `git add news/ analysis/daily/ analysis/weekly/ && git commit -m "Add interpellation-debates articles and analysis artifacts"`
...
- ✅ `safeoutputs___noop` ONLY if genuinely no new interpellations
...
**CRITICAL: Run the analysis pipeline BEFORE article generation.** This downloads data from riksdag-regering-mcp, runs all 9 analysis steps (classification, risk assessment, SWOT, threat analysis, stakeholder perspectives, significance scoring, cross-references, synthesis), and writes structured artifacts to `analysis/daily/YYYY-MM-DD/interpellations/`. Article generators will then consume these artifacts for enrichment.
