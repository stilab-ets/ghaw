...
If articles for `$ARTICLE_DATE` + `interpellations` already exist **and** `force_generation=false`:

- Still run the full analysis pipeline (modules 03 → 04 → 05).
- Commit the analysis.
- Open the single PR with title `📊 Analysis Only — Interpellation Debates — $ARTICLE_DATE` and label `analysis-only`.
