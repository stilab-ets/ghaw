...
- **Rendered HTML**: `news/$ARTICLE_DATE-week-ahead-{en,sv}.html` (produced by `scripts/render-articles.ts`)
...
At the start of every run, the pre-flight check in `03-data-download.md` detects whether `analysis/daily/$ARTICLE_DATE/week-ahead/` already contains all **23 required artifacts**:
