...
# news-month-in-review run #24954208628 fixed in #1444. The 90-day
...
tools:
  ...
  cache-memory:
    key: news-quarter-ahead-${{ github.repository_owner }}
    retention-days: 7
...
safe-outputs:
  ...
  create-pull-request:
    title-prefix: "[news] "
    labels: [agentic-news, analysis-data, "type:quarter-ahead"]
    draft: false
    expires: 14d
...
engine:
  ...
  max-continuations: 1
---
# 📰 EU Parliament Quarter Ahead — Unified Workflow
...
| Parameter | Value |
|-----------|-------|
| `ARTICLE_TYPE_SLUG` | `quarter-ahead` |
| Family | **Unified** (Stages A → B → C → D → E in one workflow) |
| Data window | next 90 days |
| Primary feeds | `get_plenary_sessions` per-month fan-out (3 months), `get_events_feed`, `get_procedures_feed`, `get_external_documents` (Trio Presidency + Commission WP); mine forward statements from prior monthly runs. |
...
- Economic context (**IMF only** for macro/fiscal/monetary/trade) is mandatory — quarterly articles always touch macro/policy.
...
LAST_QUARTER=$(date -u -d '90 days ago' +%Y-%m-%d)
RUN_EPOCH=$(date -u +%s)
RUN_ID="quarter-ahead-run$$-$RUN_EPOCH"
...
ANALYSIS_DIR=$(scripts/resolve-analysis-dir.sh "$TODAY" quarter-ahead)
WORKFLOW_START_EPOCH=$RUN_EPOCH
echo "ARTICLE_TYPE_SLUG=quarter-ahead"                  >> "$GITHUB_ENV"
...
> always derive dates from `$TODAY` / `$LAST_WEEK` / `$LAST_QUARTER`. Never
> hard-code a year.
...
```bash
HORIZON_END=$(date -u -d '90 days' +%Y-%m-%d)
...
For each upcoming plenary session in the next 90 days, identify the session
...
top-level `articleType: quarter-ahead` and every produced file under `files.*`.
...
> **⏱️ Elapsed-Time Tripwire (quarter-ahead specific)**: At the top of
...
> straight to Stage E. The 90-day forward data window naturally pushes
...
#    Emits news/<TODAY>-quarter-ahead.en.md (canonical aggregated markdown) plus
#    news/<TODAY>-quarter-ahead-en.html (rendered article). Always
...
- `base: "main"`
- `head: "news/${TODAY}-quarter-ahead-${RUN_ID}"`
- `title: "[news] quarter-ahead — ${TODAY} (run ${RUN_ID})"` *(append `(analysis-only)` suffix when `GATE_RESULT=ANALYSIS_ONLY`)*
...
- Never author article prose — the deterministic renderer owns Stage D.
  Agent edits to `news/<date>-quarter-ahead.en.md` after `npm run generate-article`
  exits are forbidden.
