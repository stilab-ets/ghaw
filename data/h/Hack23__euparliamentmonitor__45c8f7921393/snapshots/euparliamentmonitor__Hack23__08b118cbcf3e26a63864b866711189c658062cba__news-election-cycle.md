...
imports:
  ...
  - uses: shared/config/news-pat-pr-fallback.md
    with:
      slug: election-cycle
      workflowName: "News: EU Parliament Election Cycle — Unified"
...
steps:
  ...
  - name: Pre-fetch EP feeds (deterministic Stage A)
    run: bash scripts/prefetch-ep-feeds.sh election-cycle procedures documents external-documents events
...
<!-- Date Context + Stages B → E + 🚫 Never section: imported from
     shared/prompts/news-unified-stages.md (with slug: election-cycle). -->
