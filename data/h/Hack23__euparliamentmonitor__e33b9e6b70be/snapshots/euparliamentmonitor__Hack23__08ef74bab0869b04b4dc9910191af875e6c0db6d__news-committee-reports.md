...
> **⚠️ CRITICAL**: This workflow creates article files in the `news/` directory and analysis artifacts in the `analysis/daily/` directory. Except for the limited `src/` or `scripts/` corrections explicitly allowed below, you MUST NOT modify any other files.
...
- **Minutes 45–50**: Validate and finalize output — run HTML and WCAG checks, verify metadata, prose ratio check, verify zero markers remain. **🚨 PR MUST be created by minute 50 (HARD DEADLINE).**
