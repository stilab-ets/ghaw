...
RUN_EPOCH=$(date -u +%s)
RUN_ID="committee-reports-run$$-$RUN_EPOCH"
...
ANALYSIS_DIR=$(scripts/resolve-analysis-dir.sh "$TODAY" committee-reports)
...
```
Stage A · Data Collection (≤ 5 min — minute 0–5)
  → Stage B · Analysis (Pass 1 + Pass 2, 15–18 min HARD CEILING — minute 5–23)
    → Stage C · Completeness Gate (agent-side readback, ≤ 2 min — minute 23–25) — BLOCKING; minute-25 elapsed-time tripwire forces ANALYSIS_ONLY
      → Stage D · Article Render (npm run generate-article — deterministic, ≤ 2 min — minute 25–27)
        → Stage E · Single PR (≤ 1–2 min — minute ≤ 28; exactly once)
```
...
#    Emits news/<TODAY>-committee-reports.en.md (canonical aggregated markdown) plus
#    news/<TODAY>-committee-reports-en.html (rendered article). Idempotent — skips
...
- `title: "[news] committee-reports — ${TODAY} (run ${RUN_ID})"` *(append `(analysis-only)` suffix when `GATE_RESULT=ANALYSIS_ONLY`)*
...
- Never author article prose — the deterministic renderer owns Stage D.
  Agent edits to `news/<date>-committee-reports.en.md` after `npm run generate-article`
  exits are forbidden.
