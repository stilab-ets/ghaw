...
Retrospective 30-day political intelligence review synthesising longitudinal patterns. Tier-C reference-grade output (14 artifacts). Core languages EN, SV.
...
If articles for `$ARTICLE_DATE` + `monthly-review` already exist **and** `force_generation=false`:

- Still run the full analysis pipeline (modules 03 → 04 → 05).
- Commit the analysis.
- Open the single PR with title `📊 Analysis Only — Monthly Review — $ARTICLE_DATE` and label `analysis-only`.
