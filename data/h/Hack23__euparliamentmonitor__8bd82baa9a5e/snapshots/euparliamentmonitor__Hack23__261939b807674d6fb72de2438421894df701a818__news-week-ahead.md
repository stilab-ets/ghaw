...
RUN_EPOCH=$(date -u +%s)
RUN_ID="week-ahead-run$$-$RUN_EPOCH"
...
ANALYSIS_DIR=$(scripts/resolve-analysis-dir.sh "$TODAY" week-ahead)
...
- `title: "[news] week-ahead — ${TODAY} (run ${RUN_ID})"` *(append `(analysis-only)` suffix when `GATE_RESULT=ANALYSIS_ONLY`)*
...
- Never author article prose — the deterministic renderer owns Stage D.
  Agent edits to `news/<date>-week-ahead.en.md` after `npm run generate-article`
  exits are forbidden.
