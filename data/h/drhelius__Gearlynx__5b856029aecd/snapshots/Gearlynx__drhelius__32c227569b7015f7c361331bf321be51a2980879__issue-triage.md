---
name: Issue Triage
description: Triage newly opened Gearlynx issues by applying the correct repository labels.
on:
  issues:
    types: [opened]
    lock-for-agent: true
...
4. Post a clarification comment only when the issue is likely a `bug`, the next engineering action depends on Gearlynx version or platform/frontend, and the issue body does not already provide that information.
5. When asking, ask only for Gearlynx version and platform/frontend. Do not ask for ROM hashes, logs, screenshots, save files, long reproduction forms, or extra context during initial triage.
...
- `bug`: broken behavior in an already-supported Gearlynx feature, mapper, platform, build, package, or workflow. Use this for regressions, crashes, incorrect emulation, save-state problems, audio/video/input defects, frontend bugs, and release or CI breakages caused by this repository.
...
- `question`: a user question, support request, usage question, or troubleshooting question where the user is asking how something works or how to use Gearlynx.
- `not a bug`: expected behavior, invalid or corrupted ROM/save data, a known hardware limitation, an external service problem, spam/test content, or behavior caused by user environment rather than Gearlynx.
...
### Phase 4: Gearlynx-Specific Checks

For game compatibility reports, separate bugs from unsupported cartridge hardware:

1. Treat likely unsupported cartridge banking or hardware variants as `feature request`. Clues include unusual cartridge banking, homebrew or prototype cartridge behavior, special cartridge hardware, or a game that fails only because Gearlynx has no implementation for that cartridge type.
...
3. Treat bad dumps, patched/trainer ROM issues without clean-ROM evidence, invalid save files, or expected Atari Lynx hardware behavior as `not a bug` when the evidence is clear.
4. Use `needs info` when the report says only that a game does not run but omits the exact ROM title/region/revision, ROM hash, Gearlynx version, platform, steps, or whether a clean dump was tested.
...
4. Use a clearly automated but readable close-comment format, for example: `Automated triage result: this issue appears to duplicate #123 because it describes the same affected game and mapper symptom. This issue is being closed as a duplicate; comment if this report describes a different case.`
...
3. For clarification comments, use a clearly automated but readable format, for example: `Automated triage request: please provide the Gearlynx version and platform/frontend so this bug report can be classified correctly.`

## Style

Keep comments brief, neutral, and explicitly automated, but write in complete readable sentences rather than terse status fragments. Do not use first person, apologies, thanks, conversational warmth, or human-like sign-offs. Do not expose secrets, private resource identifiers, tokens, generated hostnames, or user-sensitive paths. Redact sensitive values as `<redacted>`.