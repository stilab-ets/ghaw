...
You are auditing the GitHub Wiki documentation for the **db-mcp** project — a robust SQLite MCP server with 170+ specialized tools. Your job is to check if the wiki documentation is accurate and consistent with recent code changes.
...
`git clone https://github.com/neverinfamous/db-mcp.wiki.git /tmp/wiki`
...
- **Configuration.md** / **Quick-Start.md** — CLI flags table and env var list must match `src/cli.ts` and `.env.example`.
- **Home.md** — Module tree must match current `src/` directory structure or component layout.
- **Code-Mode.md** — API groups table must match `src/filtering/tool-constants.ts` TOOL_GROUPS. Verify documentation of dynamic TS injection and runtime `.schema()` introspection.
- **Tool-Reference.md** — Tool parameters, counts, and type lists must match source schemas and code map.
...
- Feature descriptions and tool/resource counts across `Tool-Reference.md` and `Home.md`.
- Environment variable documentation across `Configuration.md` and `Quick-Start.md`.
