---
name: "News: EU Parliament Month Ahead — Unified"
description: Generates a single PR containing analysis artifacts and the rendered month-ahead article (Stages A → B → C → D → E in one workflow).
strict: false
on:
  schedule:
    - cron: "0 8 1 * *"  # 1st of each month around 08:00 UTC
  workflow_dispatch:
    inputs:
      force_generation:
...
# news-month-in-review run #24954208628 fixed in #1444. The 30-day
...
tools:
  ...
  cache-memory:
    key: news-month-ahead-${{ github.repository_owner }}
    retention-days: 7
...
safe-outputs:
  ...
  create-pull-request:
    title-prefix: "[news] "
    labels: [agentic-news, analysis-data, "type:month-ahead"]
    draft: false
    expires: 14d
    ...
    steps:
      ...
      - name: Run host-side PAT PR fallback
        env:
          ...
          GH_AW_PAT_FALLBACK_SLUG: month-ahead
          GH_AW_PAT_FALLBACK_WORKFLOW_NAME: "News: EU Parliament Month Ahead — Unified"
...
engine:
  ...
  max-continuations: 1
---
# 📰 EU Parliament Month Ahead — Unified Workflow
...
| Parameter | Value |
|-----------|-------|
| `ARTICLE_TYPE_SLUG` | `month-ahead` |
| Family | **Unified** (Stages A → B → C → D → E in one workflow) |
| Data window | next 30 days |
...
| Stage A budget | **≤ 4 min** (tighter than other workflows — 30-day forward data window is larger) |
...
RUN_EPOCH=$(date -u +%s)
RUN_ID="month-ahead-run$$-$RUN_EPOCH"
...
ANALYSIS_DIR=$(scripts/resolve-analysis-dir.sh "$TODAY" month-ahead)
WORKFLOW_START_EPOCH=$RUN_EPOCH
echo "ARTICLE_TYPE_SLUG=month-ahead"                  >> "$GITHUB_ENV"
...
top-level `articleType: month-ahead` and every produced file under `files.*`.
...
> **⏱️ Elapsed-Time Tripwire (month-ahead specific)**: At the top of
...
#    news/<TODAY>-month-ahead-en.html (rendered article). Always
...
- `base: "main"`
- `head: "news/${TODAY}-month-ahead-${RUN_ID}"`
- `title: "[news] month-ahead — ${TODAY} (run ${RUN_ID})"` *(append `(analysis-only)` suffix when `GATE_RESULT=ANALYSIS_ONLY`)*
...
- Never author article prose — the deterministic renderer owns Stage D.
  Agent edits to `news/<date>-month-ahead.en.md` after `npm run generate-article`
  exits are forbidden.
